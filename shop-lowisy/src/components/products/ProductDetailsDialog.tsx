...
export default function ProductDetailsDialog({ setDialogOpen, product }) {
  const [open, setOpen] = React.useState(false);
  const [scroll, setScroll] = React.useState<DialogProps["scroll"]>("paper");
  const [hover, setHover] = React.useState("outlined");
  const router = useRouter();
  const { name, packagingContent, shortDescription, longDescription } = product;
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: "flex-row" }}>
            <H6>{name}</H6>
            <H6>{packagingContent}</H6>
            <H6>{shortDescription}</H6>
            <H6>{longDescription.replace(/(<([^>]+)>)/gi, "")}</H6>
          </Box>
        </DialogContentText>
      </DialogContent>
      {/* </Dialog> */}
