...
SubCategoriesList.getLayout = function getLayout(page: React.ReactElement) {
...
export default function SubCategoriesList({
  ...
  //     refetch,
  //   } = useQuery<any>(
  //     ['get_categories', page, rowsPerPage, filterName, orderBy, order],
  //     () =>
  //       getCategories({ page, limit: rowsPerPage, searchQuery: filterName, sortBy: orderBy, order }),
  //     {
  ...
  useEffect(() => {
    setTotalCategories(subCategories?.length);
    refetch();
  }, [subCategories]);
  ...
  const isNotFound =
    (!subCategories?.length && !!filterName) || (!isLoading && !subCategories?.length);
  const { translate } = useLocales();
  return (
    <Container maxWidth={themeStretch ? false : 'lg'}>
      <Typography variant="h6" sx={{ m: 2 }}>
        {translate('Available subcategories')}
      </Typography>
        ...
        <Scrollbar>
          <TableContainer sx={{ minWidth: 960, position: 'relative' }}>
            {selected.length > 0 && (
              <TableSelectedActions
                ...
                onSelectAllRows={(checked) =>
                  onSelectAllRows(
                    checked,
                    subCategories.map((row: any) => row.id)
                  )
                }
                actions={
                  <>
            ...
            <Table size={dense ? 'small' : 'medium'}>
              ...
              <TableBody>
                {(isLoading ? [...Array(rowsPerPage)] : subCategories)
