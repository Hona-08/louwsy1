...
import { getShopProductByProductName } from 'src/api/products';
...
export default function EcommerceProductDetails() {
  ...
  const { shopId, name } = query;

  //console.log('QUERY', query);

  const productName: string = noCase(name!.toString());
  //console.log(shopId);
  //console.log(productName);

  const { data: product, isError } = useQuery<any>(['get_shop_product'], () =>
    getShopProductByProductName(shopId as string, productName as string)
  );

  // //console.log(noCase(productName));
  ...
  return (
    <Page title="Ecommerce: Product Details">
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} sx={{ mt: 20 }}>
          ...
          <>
            ...
            <Card>
              <TabContext value={value}>
                ...
                <TabPanel value="1">
                  <Box sx={{ p: 3 }}>
                    <Markdown children={product.shortDescription} />
                  </Box>
                </TabPanel>
                <TabPanel value="2">
        ...
        {isError && <Typography variant="h6">{translate('product_not_found')}</Typography>}
      </Container>
    </Page>
  );
}
